...
> 本文档覆盖面试权重 10% 的数据库方向知识，按"ORM 框架 → 缓存中间件 → 连接池管理 → 其他框架解"的逻辑递进。与 Go 核心基础、Web 框架、微服务等方向的知识关联见[总览文档](go语言知识规划——总览.md)的跨域链路。
...
**链路 2：事务 → 微服务**：
GORM 本地事务 → MySQL ACID → 分布式事务（TCC / Saga / 2PC） → [Go-Zero/DTM 分布式事务框架](go语言知识规划——微服务.md)

**链路 3：缓存 → Web 框架**：
go-redis 缓存操作 → Cache-Aside 模式 → Gin Web 框架中间件缓存（本地缓存 + Redis 二级缓存） → [Web 框架中间件链](go语言知识规划——web框架.md)

**链路 4：连接池 → 测试工具**：
连接池配置和监控 → pprof 分析连接池阻塞 → 压测发现连接瓶颈 → [测试工具 pprof](go语言知识规划——测试调试.md)

**链路 5：选型 → 项目架构**：
GORM vs sqlx vs ent 选型 → 项目阶段（快速原型 vs 大型项目） → 团队经验（SQL 熟悉度 vs codegen 接受度） → [微服务项目架构](go语言知识规划——微服务.md)

**链路 6：GORM 接口抽象 → 依赖注入**：
GORM 的 `Tabler` 接口（`TableName() string`） → Hook 接口（`BeforeCreate`/`AfterFind` 等） → 接口隐式实现设计 → [Wire 依赖注入](go语言知识规划——核心.md) → 抽象模式贯穿线（见总览文档链路 D）
