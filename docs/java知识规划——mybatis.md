...
- 深分页 `LIMIT 1000000, 20` 仍需优化（参考 [MySQL 游标分页](java知识规划——mysql.md#47-sql-优化策略)）
...
> **关联知识点**：设计模式（模板方法/工厂方法/装饰器）→ [核心 设计模式](java知识规划——核心.md#16-设计模式) / CachingExecutor → 3.4 二级缓存 / Plugin 链 → 3.6 Interceptor / Spring 整合 → SqlSessionTemplate → 3.3 SqlSessionFactory
...
> **关联知识点**：代码生成 → [Spring Boot Starter 规范](java知识规划——spring.md#23-spring-boot-核心机制)（自动配置 / @ConditionalOnClass）/ MyBatis-Plus → 3.7 / Maven Plugin → maven 构建生命周期
