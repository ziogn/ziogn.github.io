...
created: 2026-07-23 12:20
updated: 2026-07-23 12:20
version: 0.0.1
author: ziogn
...
> 本文档是 Java 知识规划体系的理论补全，按"理论基石 → 一致性协议 → ID → 事务 → 锁 → 存储 → RPC → 可观测 → 配置 → 设计思维"递进。与 [Spring Cloud 微服务](java知识规划——springCloud微服务.md)方向的关系：微服务是实践，分布式理论是底层原理。建议先读微服务方向再读本文档。
...
> **关联知识点**：AP/CP 选型 → [Nacos 注册中心 CAP 权衡](java知识规划——springCloud微服务.md#31-nacos注册中心--配置中心) / BASE 最终一致 → [MySQL 主从同步](java知识规划——mysql.md#49-主从复制binlog) 最终一致
...
> **关联知识点**：Raft → [Sentinel 故障转移 Raft](java知识规划——springCloud微服务.md#32-gateway-路由网关) 选举 / Gossip → [Redis Cluster 通信](java知识规划——redis.md#63-redis-主从--哨兵--集群) / 2PC → [Seata AT 全局锁](java知识规划——springCloud微服务.md#35-seata-分布式事务) / Paxos → [ZooKeeper ZAB 协议](java知识规划——redis.md#65-redis-分布式锁)
...
> **关联知识点**：Snowflake → B+Tree 顺序写入 / Leaf → 号段预取思想与 [MyBatis BatchExecutor](java知识规划——mybatis.md#311-mybatis-executor-三种实现) 批量思想一致
...
Seata AT 是业界最广泛使用的 Java 分布式事务方案，已在 [微服务文档](java知识规划——springCloud微服务.md#35-seata-分布式事务) 详述。本文仅提炼其理论本质：
...
> **关联知识点**：Seata AT → [微服务文档 Seata 全局锁](java知识规划——springCloud微服务.md#35-seata-分布式事务) / 事务消息 → [RocketMQ 事务消息回查](java知识规划——springCloud微服务.md#36-rocketmq-消息中间件) / XA → [MySQL 两阶段提交](java知识规划——mysql.md#410-innodb-内存结构buffer-pool--redo-log--undo-log--binlog)
...
> **关联知识点**：Redis 分布式锁 → [Redis 分布式锁深度](java知识规划——redis.md#65-redis-分布式锁) / ZK 临时节点 → [核心 会话超时](java知识规划——核心.md#13-并发编程) 类比
...
> **关联知识点**：Hash 分片 → [Redis Cluster 16384 槽](java知识规划——redis.md#63-redis-主从--哨兵--集群) / 一致性哈希 → 分布式缓存扩容
...
> **关联知识点**：Netty → [核心 NIO/Netty 深度](java知识规划——核心.md#15-nio-与-netty) / Dubbo 协议 → [微服务 Dubbo 深度](java知识规划——springCloud微服务.md#34-dubbo-rpc-框架) / gRPC → HTTP/2 多路复用
...
> **关联知识点**：TraceId 透传 → [核心 ThreadLocal](java知识规划——核心.md#13-并发编程) / 采样策略 → [SkyWalking 采样](java知识规划——springCloud微服务.md#37-链路追踪micrometer-tracing--skywalking)
...
> **关联知识点**：Bootstrap 上下文 → [Spring Cloud 配置加载](java知识规划——springCloud微服务.md#39-bootstrap-上下文与配置加载) / Nacos 2.0 gRPC → [核心 Netty](java知识规划——核心.md#15-nio-与-netty)
...
> **关联知识点**：熔断 → [Sentinel 熔断状态机](java知识规划——springCloud微服务.md#33-sentinel-限流与熔断) / 线程池隔离 → [核心 线程池](java知识规划——核心.md#13-并发编程) / 降级开关 → [配置中心](java知识规划——springCloud微服务.md#39-bootstrap-上下文与配置加载)
